...
export default function SignInScreen({ navigation }) {
  ...
  const { data, loading, user } = useSelector((state) => {
    return state.User;
  });
  ...
  _storeData = async () => {
    try {
      ...
      if (isSelected) await AsyncStorage.setItem("remember", "true");
  ...
  _retrieveData = async () => {
    try {
      ...
      if (email !== null && password !== null && remember === "true") {
        ...
        if (remember === "true") setSelection(true);
  ...
  const _submit = async () => {
    ...
    await Api.checkLogin(dataTemp.email, dataTemp.password).then((res) => {
      if (
        res != "Invalid Password!" &&
        res != "Login failed! Account was not registered!"
      ) {
        res.forEach(async (element) => {
          if (element.email === dataTemp.email) {
            ...
            await JWTApi.getToken(element.userID).then((res) => {
          ...
          <View style={{ marginTop: 5 }}>
            ...
            <View style={styles.passwordView}>
              ...
              <Ionicons
                ...
                color="black"
                onPress={() =>
                  setData({ ...dataTemp, showPassword: !dataTemp.showPassword })
                }
              />
            </View>
          </View>
